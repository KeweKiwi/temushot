    ...
    <section id="blog" className="py-12 md:py-16">
      <div className="section-shell">
        <Reveal className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          ...
          <Link href={`/blog/${articles[0].slug}`} className="inline-flex w-fit items-center gap-2 rounded-full border border-orange/30 bg-white/60 px-5 py-3 text-sm font-black text-burnt transition hover:bg-white">
          ...
          {articles.map((article, index) => (
            <Reveal key={article.title} delay={index * 0.06}>
              <Link
                ...
                className="group block h-full rounded-[1.5rem] border border-orange/12 bg-white/72 p-6 shadow-soft transition hover:-translate-y-2 hover:bg-white"
              >
                ...
                <span className="mt-5 inline-flex items-center gap-2 text-sm font-black text-orange">
